...
function ManageAllManga() {
  ...
  const { isError, allMangaManager } = useSelector(
    (state) => state.getAllMangaManager
  );
  const oldMangasRef = useRef();
  useEffect(() => {
    if (allMangaManager && allMangaManager?.length > 0) {
      oldMangasRef.current = allMangaManager;
    }
  }, [allMangaManager]);
  ...
  // useEffect
  useEffect(() => {
    dispatch(getAllMangaManagerAction());
    setRecords();
  ...
  const handleFilter = (event) => {
    const newData = allMangaManager?.filter((row) => {
  ...
  const handleDeleteManga = () => {
    ...
    if (
      ...
      dispatch(deleteMangasAction(id, isAdmin));
    }
  };
    ...
    <AdminLayout>
      <div className="account-info clearfix">
        <div className="account-form clearfix">
          <div className="row">
            {allMangaManager?.length > 0 || oldMangas?.length > 0 ? (
              <div className="panel-body " style={{ padding: "0 15px" }}>
                ...
                <DataTable
                  columns={columns}
                  data={records ? records : allMangaManager || oldMangas}
                  selectableRows
                  fixedHeader
                  pagination
